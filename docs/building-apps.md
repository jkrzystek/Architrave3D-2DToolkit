...
10. **Background heavy operations** — Long computations like Texture Baking, CSG, and File I/O should be sent to a `std::thread::spawn` background thread using `std::sync::mpsc` channels to return results. This prevents freezing the main Winit/Egui event loop and avoids "Application Not Responding" crashes on Windows.
11. **Egui + Wgpu Viewport Layout** — When embedding a 3D Wgpu viewport behind Egui, declare `egui::TopBottomPanel` and `egui::SidePanel` at the root context *first*, and declare `egui::CentralPanel::default().frame(egui::Frame::none())` *last*. This makes the central panel transparent, revealing the 3D model underneath, while correctly capturing `ui.allocate_response(..., Sense::drag())` for camera controls.
12. **Distinctive Clear Colors** — Set your `wgpu::LoadOp::Clear` color to something distinct (like light blue) so you can easily distinguish the 3D canvas from the Egui theme background.
13. **File Logging for GUI Apps** — When building a standard Windows desktop app (which hides the terminal), implement the `log::Log` trait to append all `log::info!` statements to a local `app.log` file with a 1MB size cap. This guarantees you have persistent crash and debug logs.
14. **Dynamic Surface Formats** — Never hardcode your Wgpu `RenderPipeline` color format (e.g., `Rgba8Unorm`). Always dynamically request `surface.get_capabilities(&adapter).formats[0]` to prevent Vulkan/Metal/DX12 validation panics across Linux, Mac, and Windows.
